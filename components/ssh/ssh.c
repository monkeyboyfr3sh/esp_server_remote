...
void print_command(ssh_cmd_input_t *ssh_commnad)
{
	printf("%s%s%s@%s%s\e[0m: ", COLOR_1,ssh_commnad->username,COLOR_2,COLOR_1,ssh_commnad->host);
	printf("%s > \r\n",ssh_commnad->command);
}
...
void ssh_command_task(void *pvParameters)
{
	...
	if(ssh_commnad->password){
		password =  ssh_commnad->password;
	}
	...
	ESP_LOGI(TAG, "CONFIG_SSH_HOST=%s", ssh_commnad->host);
	ESP_LOGI(TAG, "CONFIG_SSH_PORT=%d", CONFIG_SSH_PORT);
	sin.sin_family = AF_INET;
	//sin.sin_port = htons(22);
	sin.sin_port = htons(CONFIG_SSH_PORT);
	sin.sin_addr.s_addr = inet_addr(ssh_commnad->host);
	ESP_LOGI(TAG, "sin.sin_addr.s_addr=%x", sin.sin_addr.s_addr);
	if (sin.sin_addr.s_addr == 0xffffffff) {
		struct hostent *hp;
		hp = gethostbyname(ssh_commnad->host);
		if (hp == NULL) {
			ESP_LOGE(TAG, "gethostbyname fail %s", ssh_commnad->host);
			while(1) { vTaskDelay(1); }
		}
	...
	if(libssh2_userauth_password(session, ssh_commnad->username, password)) {
		ESP_LOGE(TAG, "Authentication by password failed.");
		ESP_LOGE(TAG, "Authentication username : [%s].", ssh_commnad->username);
		while(1) { vTaskDelay(1); }
	}
...
void ssh_task(void *pvParameters)
{
	...
	ESP_LOGI(TAG, "Start task_parameter=%s", task_parameter);
	ESP_LOGW(TAG,"This does nothing...");
// 	// SSH Staff
// 	int sock;
// 	struct sockaddr_in sin;
// 	LIBSSH2_SESSION *session;
